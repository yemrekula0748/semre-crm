...
function saveOrdersToDB($orders, $existingOrders, $db) {
    ...
    foreach ($orders as $order) {
        ...
        $sql = "INSERT INTO ikas_siparisler (order_id, orderNumber, totalFinalPrice, createdAt,
                 customer_firstname, customer_lastname, city, productNames, phone, district, shippingAddress, billingAddress, orderLineItems)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $params = [
            ...
            date("Y-m-d H:i:s", intval($order['createdAt'] / 1000)), // Milisaniyeden saniyeye dönüşüm
            ...
            json_encode($order['shippingAddress']),
            json_encode($order['billingAddress']),
            json_encode($order['orderLineItems'])
        ];
        $types = "isdsdssssssss";
