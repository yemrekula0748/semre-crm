...
try {
    ...
    $sqlSelect = "SELECT id, sales_invoice_id 
                 FROM siparisler 
                 WHERE resmilestir = 1 
                 AND sales_invoice_id IS NOT NULL 
                 AND iptalmi = 0 
                 AND parasut_resmilesme_durumu = 0
                 AND hangikargo = 'Sevim Aydın - PTT'";
                 
    $result = $db->query($sqlSelect);
    
    // Token al
    $tokenSql = "SELECT parasut_token_sevimaydin FROM parasut_token WHERE id = 1 LIMIT 1";
    $tokenResult = $db->query($tokenSql);
    $tokenRow = $db->fetchAssoc($tokenResult);
    $accessToken = $tokenRow['parasut_token_sevimaydin'] ?? null;
